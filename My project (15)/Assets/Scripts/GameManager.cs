...
public class GameManager : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        //Skor 50 ve katlarý olunca hýz artsýn
        if (score >= 50)
        {
            Time.timeScale = 1.3f; // Oyun hýzýný arttýrýr.
        }
        if (score >= 100 )
        {
            Time.timeScale = 1.5f; // Oyun hýzýný arttýrýr.
        }
        if (score >= 150)
        {
            Time.timeScale = 1.7f; // Oyun hýzýný arttýrýr.
        }
        if (score >= 200)
        {
            Time.timeScale = 1.9f; // Oyun hýzýný arttýrýr.
        }
        if (score >= 250)
        {
            Time.timeScale = 2f; // Oyun hýzýný arttýrýr.
        }
        if (score >= 300)
        {
            Time.timeScale = 2.3f; // Oyun hýzýný arttýrýr.
        }
    }
